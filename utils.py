...
def save_weights(modules_list: list, ckpt_path, suffix):
    ...
    state = {"tt": None, "bn": None, "st": None, "dfs": None}
...
def load_weights(modules_list: list, ckpt_path, suffix):
    ...
    new_state = {"tt": None, "bn": None, "st": None, "dfs": None}
...
class utils_3D:
    ...
    def organized_pc_to_unorganized_pc(organized_pc):
        return organized_pc.reshape(
            organized_pc.shape[0] * organized_pc.shape[1], organized_pc.shape[2]
        )
    ...
    def resize_organized_pc(organized_pc, img_size, tensor_out=True):  # 224
        torch_organized_pc = (
            torch.tensor(organized_pc).permute(2, 0, 1).unsqueeze(dim=0)
        )
        torch_resized_organized_pc = torch.nn.functional.interpolate(
            torch_organized_pc, size=img_size, mode="nearest"
        )
        if tensor_out:
